...
> - **(d) Model Store staging is GPU-only — CPU testbed cannot measure it.** Declaration succeeds on CPU endpoints, but the platform never stages files to a CPU worker: confirmed live across multiple runs incl. the declare-before-any-worker pattern and a forced-cold worker (`workersMax 0→1`, `modelReferences` re-carried) — every CPU worker saw `no models--…--… dir under /runpod-volume/huggingface-cache/hub` (0 warm jobs). Matches Global-Volume's CPU restriction. The modelstore mechanism (declaration + worker resolution) is correct and shipped; a real measurement needs GPU endpoints (out of scope for this CPU download-timing testbed).
> - **Consequence (resolved):** Task 16's default Flash `volume=` branch is correct (keep it; the REST helper now sends a User-Agent). Task 19 now **automates** the cached-model declaration via the `saveEndpoint`+`modelReferences` GraphQL path (Task 19 Step 3 branch (ii), realized in `provision/modelstore_api.py`), with the manual console step as the fallback and the `[modelstore.endpoints]` reuse path retained. Model Store *measurement* is GPU-only (finding (d)); shim (8.2×) and volumecache (2.7×) are the CPU-measurable mechanisms.
